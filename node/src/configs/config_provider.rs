...
impl Provider for CustomEnvProvider {
    fn metadata(&self) -> Metadata {
        Metadata::named("iggy-node config")
    }

    fn data(&self) -> Result<FigmentMap<Profile, Dict>, figment::Error> {
        let default_config = toml::to_string(&SystemConfig::default())
            .expect("Cannot serialize default SystemConfig. Something's terribly wrong.");
...
impl FileConfigProvider {
    pub async fn load_config(&self) -> Result<SystemConfig, SystemError> {
        info!("Loading config from path: '{}'...", self.path);
        if !file_exists(&self.path) {
            return Err(SystemError::ConfigNotFound(format!(
        ...
        let config_builder = match extension {
            ...
            e => {
                return Err(SystemError::ConfigNotFound(format!("Cannot load configuration: invalid file extension: {e}, only .toml is supported.")));
            }
        };
        ...
        match config_result {
            ...
            Err(figment_error) => Err(SystemError::ConfigInvalid(format!(
